...
saved_trend <- saved_trends[which(saved_trends$species == sp1 &
   ...
   sp_file_name <- paste0(spf,"-gamma-t")
      ...
      if(file.exists(paste0(output_dir,"/",sp_file_name,"_fit_add.RData"))){
        ...
        dts$count <- stan_data_sim$count
         ...
         csvfl <- paste0(output_dir,"/",csvfl2[grepl(csvfl2,pattern = ".csv",fixed = TRUE)])
         ...
         #    theme_minimal()
         # 
         # pdf(paste0("Figures/",sp,"gamma-t","_obs_by_alpha.pdf"),
         #     width = 11,
         #     height = 11)
         # print(obs_by_alpha)
         # dev.off()
 ...
 # extracting the seasonal smooth ------------------------------------------
         ...
         #    labs(title = paste(sp,"gamma-t","_Survey-wide trajectory (full and smooth) with obs means"))+
         ...
         #    geom_ribbon(aes(ymin = lci,ymax = uci),alpha = 0.2)+
         #    geom_line(aes(colour = parm))+
         #    labs(title = paste(sp,"gamma-t","_Survey-wide trajectory (full and smooth)"))+
         #    my_col2_traj+
         #    xlab("")+
         ...
         #                       true_trend = 100*(exp(mean(strats_xy$b1))-1),
         #                       North_position = 0)
         ...
         #   
         # # pdf(paste0("Figures/Trend_Heat_maps_",sp,"gamma-t","_cmd_.pdf"),
         # #     width = 11,
         # #     height = 8.5)
...
write.csv(trendsoutsplit[[1]],paste0("trends/All_strata_Simulated_","gamma-t","_level_trends.csv"),row.names = FALSE)
#write.csv(trendsoutsplit[[2]],paste0("trends/All_region_Simulated_","gamma-t","_level_trends.csv"),row.names = FALSE)
...
save(list = c("trend_maps_1980",
              ...
              #"season_graphs",
              #"loo_ic"),
     file = paste0("Figures/All_Simulated_","gamma-t","_stored_maps.RData"))




# Trend maps and comparisons --------------------------------------------------------------
pdf(file = paste0("Figures/All_Simulated_","gamma-t","_trend_maps.pdf"),
    width = 9, height = 6.5)
for(sp1 in sps){
...
pdf(file = paste0("Figures/All_simulated","gamma-t","_long_term_3Gen_trends.pdf"),
    height = 9,
    width = 6.5)
print(lt3_tplot)
dev.off()
...
pdf(file = paste0("Figures/All_simulated_comparison","gamma-t","_long_term_3Gen_trends.pdf"),
    height = 9,
    width = 13.5)
...
pdf(file = paste0("Figures/All_simulated_comparison_Cosewic_etc_","gamma-t","_long_term_3Gen_trends.pdf"),
    height = 9,
    width = 13.5)
...
pdf(file = paste0("Figures/All_simulated_comparison_overdispersion_","gamma-t","_long_term_3Gen_trends.pdf"),
    height = 9,
    width = 13.5)
