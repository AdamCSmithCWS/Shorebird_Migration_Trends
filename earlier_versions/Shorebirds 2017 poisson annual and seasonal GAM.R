...
for (sp2 in storun[(ww):(ww+9)]) {
  ...
  if (length(stratkeep) > 2){ ### only runs bugs model for species with 3 or more strata
      ...
      recenter = floor(diff(c(1,nyears))/2)
      rescale = 10 # this generates a year variable with sd ~ 1 because of the ~50 years in the time-series
      dts$yearscale = (dts$yearx-recenter)/rescale
      
      scaledyear = seq(min(dts$yearscale),max(dts$yearscale),length = nyears)
      ymin = min(dts$Year)
      ymax = max(dts$Year)
      ...
      if(ymin != 1974){
        ...
        ymin = 1974
        nyears = length(scaledyear)
      }
      yminsc = scaledyear[as.character(ymin)]
      ymaxsc = scaledyear[as.character(ymax)]
      # if(ymin != 1974){
      #   yminpred = 1974
      #   yminscpred = scaledyear[as.character(1974)]
      # }
      ############ Annual GAM basis function
      
      knotsY<- seq(yminsc,ymaxsc,length=(nknotsy+2))[-c(1,nknotsy+2)]
      Y_K<-(abs(outer(seq(yminsc,ymaxsc,length = nyears),knotsY,"-")))^3
      Y_OMEGA_all<-(abs(outer(knotsY,knotsY,"-")))^3
      Y_svd.OMEGA_all<-svd(Y_OMEGA_all)
      Y_sqrt.OMEGA_all<-t(Y_svd.OMEGA_all$v  %*% (t(Y_svd.OMEGA_all$u)*sqrt(Y_svd.OMEGA_all$d)))
      Y.basis<-t(solve(Y_sqrt.OMEGA_all,t(Y_K)))
      ...
      save(data.jags,file = paste0(dir.spsp,"/original data object",sp2d,".RData"))
      #   
      # #inits
      # inits <- function(){
      #   
      ...
      #             "sdbeta","alpha",
      #             "r") #"u95new",
      params <- c("ste","ann","sdsite","sdyear","gof","gofnew","posdiff",
                  "nzeronew","meannew",
                  "sdbetagam","alpha",
                  "zi",
                  "sdnoise",
                  ...
                  "sdbetagamY",
                  "sdY") #"u95new",
      mod <- paste(getwd(),"/Shorebirdmodel Poisson annual and seasonal GAM.txt",sep = "")
