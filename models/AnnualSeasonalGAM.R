...
sdgam_year ~ dt(0, 0.5, 10)T(0,) # half-t prior on sd (chung et al. 2013) DOI: 10.1007/S11336-013-9328-2 places ~95% of the prior < 1.0
...
for(k in 1:nknots_year){
  ...
  for(s in 1:nstrata){
    ...
    b_year[k,s] ~ dnorm(0,taugam_year_b)
...
for(y in 1:nyears){
  for(s in 1:nstrata){
    for(j in 1:nsites[s]){
      n_sj[j,s,y] <- exp(alpha + ste[j,s] + sm_year[y,s] + vis.sm_season[76] + retrans) #site-level predictions including strata-level yearly smooths
    }#j
