...
// same as GAMYE_strata_simple but with a gamma mixture parameterization of the t-noise
...
parameters {
 ...
 real<lower=2> nu; //df for t-distribution
 real<lower=0> gtau; // gamma mixture tau
 vector[ncounts] galpha; //gamma mixture alpha
...
transformed parameters { 
  ...
  B = sdyear_gam*B_raw;
   noise = galpha / sqrt(gtau);// gamma mixture
...
model { 
  real half_nu;//gamma mixture parameters
  real gb;//gamma mixture parameters
 ...
 nu ~ gamma(2,0.1); // prior on df for t-distribution of heavy tailed site-effects from https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations#prior-for-degrees-of-freedom-in-students-t-distribution
  half_nu = 0.5*nu; //gamma mixture parameters
  gb = sdnoise*sdnoise*half_nu; //beta parameter of gamma distribution below to generate a generalized student t-distribution
  gtau ~ gamma(half_nu,gb);//gamma mixture parameters
  galpha ~ std_normal();//gamma mixture parameters
