...
for(s in 1:nstrata){
  ...
  sdnoise[s] ~ dt(0, 1, 4)T(0,) # half-t prior on sd (chung et al. 2013) DOI: 10.1007/S11336-013-9328-2 places ~95% of the prior < 3.0
...
for(y in 1:nyears){
  for(s in 1:nstrata){
    for(j in 1:nsites[s]){
      n_sj[j,s,y] <- exp(alpha[s] + ste[j,s] + sm_year[y,s] + year_effect[y,s] + mn_sm_season[s] + retrans[s])* psi #site-level predictions including strata-level yearly smooths
      n_sj_sm[j,s,y] <- exp(alpha[s] + ste[j,s] + sm_year[y,s] + mn_sm_season[s] + retrans[s])* psi #site-level predictions including strata-level yearly smooths
    ...
    # n_s_sm_a1[s,y] <- mean(n_sj_sm_a1[1:nsites[s],s,y])#stratum predictions including strata-level yearly smooths and scaled to mean across stratum sites
    # 
    ...
    # 
    # n_s_scaled[s,y] <- exp(alpha[s] + sm_year[y,s] + year_effect[y,s] + mn_sm_season[s]+ retrans[s] + retrans_j)* psi #stratum predictions including strata-level yearly smooths and on a common scale (visualisation only)
    # n_s_scaled_sm[s,y] <- exp(alpha[s] + sm_year[y,s] + mn_sm_season[s] + retrans[s] + retrans_j)* psi #stratum predictions including strata-level yearly smooths and on a common scale (visualisation only)
  ...
  N[y] <- exp(alpha_m + sm_year_B[y] + YE[y] +  retrans_m + retrans_j)* psi #continental predictions including only the hyperparameter smooth
  N_sm[y] <- exp(alpha_m + sm_year_B[y] + retrans_m + retrans_j)* psi #continental predictions including only the hyperparameter smooth
  N_comp[y] <- mean(n_s[1:nstrata,y]) #continental predictions 
  N_comp_sm[y] <- mean(n_s_sm[1:nstrata,y]) #continental predictions 
