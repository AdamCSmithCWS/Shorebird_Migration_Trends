...
model { 
 ...
 //nu ~ gamma(2,0.1); // prior on df for t-distribution of heavy tailed site-effects from https://github.com/stan-dev/stan/wiki/Prior-Choice-Recommendations
  ...
  noise_raw ~ std_normal(); // student_t(4,0,1);//extra Poisson log-normal variance
  ...
  //sum(B_raw) ~ normal(0,0.0001*nknots_year);//sum to zero constraint on GAM hyperparameters
 ...
 seas_max[2] = seas_max2;
  ...
  for(y in 1:nyears){
    ...
    for(y in 1:nyears){
      ...
      NSmooth[y] = exp(ALPHA1 + Y_pred[y] + seas_max[1] + 0.5*(sdyear^2) + 0.5*(sdalpha^2) + 0.5*(sdnoise^2) );
