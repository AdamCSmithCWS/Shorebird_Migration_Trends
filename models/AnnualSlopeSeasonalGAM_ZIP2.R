...
for(i in 1:ncounts){
	...
	elambda1[i] <- alpha[strat[i]] + ste[site[i],strat[i]] + beta[strat[i]]*(yr[i]-midyear) + 
	  year_effect[yr[i],strat[i]] + gam.sm_season[i,strat[i]] + noise[i]  ### common intercept, varying slopes, so that the site effect accounts for all of the variation in abundance.
...
for(y in 1:nyears){
  for(s in 1:nstrata){
    for(j in 1:nsites[s]){
      n_sj[j,s,y] <- exp(alpha[s] + ste[j,s] + beta[s]*(y-midyear) + year_effect[y,s] + mn_sm_season[s] + retrans[s])* psi[j,s] #site-level predictions including strata-level yearly smooths
  ...
  N[y] <- exp(alpha_m + B*(y-midyear) + YE[y] + retrans_m + retrans_j)*PSI #continental predictions including only the hyperparameter smooth
